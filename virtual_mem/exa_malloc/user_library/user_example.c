...
//First malloc and the returning malloc address is the handle.address
exa_malloc(handle1);

printf("\nhandle1: exa_malloc success with address value = %lx\n\r",handle1.address);

//Second handle initialization
handle2.smr = 0x80000200;
handle2.size = mem_size;

//second malloc
exa_malloc(handle2);
printf("\nhandle2: exa_malloc success with address value = %lx\n\r",handle2.address);

//freeing both memory segments according to handle.address
exa_free(handle1);
exa_free(handle2);
