...
//todo this needs to be set via octl
//    smr = 0x80000200;
//    writel_relaxed(smr, gr0_base + ARM_SMMU_GR0_SMR(0));
...
//    flush_dcache_poc(address,1024 *4);
    ...
    printk(KERN_INFO "pgd value: %llx %llx, \n", *(pgd + 4), *pgd);
...
static  change_pte(pte_t *ptep, pgtable_t token, unsigned long addr, void *data)
{
  ...
  printk(KERN_INFO "set OS pte value: %llx\n",*ptep);
  pte_val(pte) &= ~(u64)(3 << 8);
/*FIXME OLD_VALUE WORKI<G*/  pte_val(pte) |= (2 << 8);
//pte_val(pte) |= (0 << 8);

//extract the bits from pte
  unsigned long long pte_bits = pte_val(pte) & 0xff;
  unsigned long long pte_val_new = pte_val(pte) & 0x00;
  pte_val_new = pte_val_new >> 59; //hardwiring the 2 LS bits to 3.Possible bug?
  pte_val(pte) &= 0xFFFFFFFFFFFFFFE0;
//        pte_val(pte) |= (1 << 0);
//        pte_val(pte) |= (1 << 1);
//        pte_val(pte) |= (1 << 2);
//        pte_val(pte) |= (1 << 3);
//FIXME CHANGELOG STOPS HERE, the above are non cacheable and working
...
//  pgprot_t os_clear,os_set;
        ...
        pte_t pte = READ_ONCE(*ptep);
  printk(KERN_INFO "set total pte value:``` %llx\n",*ptep);
  //pte_val(pte) =  0xe80000A0010e4f;
  printk(KERN_INFO "total pte value has ben seet to:``` %llx\n",*ptep);

        set_pte(ptep, pte);
  // printk(KERN_INFO "change_pte: addr is %lx\n",addr);
    printk(KERN_INFO "change_pte_addr: pte is %lx\n",(unsigned long) ptep) ;
...
static int set_ttbr(long long temp)
{
  ...
  printk(KERN_INFO "Temp received in driver %llx",temp);
  ...
  printk(KERN_INFO "written page table to ttbr0 %llx \n",reg64);
  ...
  void * empty;
    ...
    int ret;
      ...
      //this needs to better that the version called by set_ttbr
			...
			printk(KERN_INFO "SMR0 vaddr is %lx\r\n",virt_to_phys(0xfd800800));
        ...
        //set the pte for a range to outer sharable.
                ...
                printk(KERN_INFO "Setting Outer Sharable for %llx\n",mem_in.address);
//				find_physical_pte(mem_in.address);
        ...
        printk(KERN_INFO "Set outersharable\n");
...
//                              find_physical_pte(mem_in.address);
        		...
        		printk(KERN_INFO "Pointing PTE to addr(could be bram)\n");
