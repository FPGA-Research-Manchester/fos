...
void soft_shrek(bus_t in_pixels[1024*1024*3], bus_t out_pixels[1024*1024*3],
                 int image_width, int image_height)
{
  ...
  for(int i = 0; i < pixelcount; i++, offset += 3) {
    ...
    u8 red   = in_pixels[offset+0];
    u8 green = in_pixels[offset+1];
    u8 blue  = in_pixels[offset+2];

    u16 tmp_pixel_g = green + 15;
    u16 tmp_pixel_b = blue + 15;

    if(((red > 95) & (green > 40) & (blue > 20))
        & ((red > tmp_pixel_g) & (red > tmp_pixel_b) & (green > blue))
        | ((red > tmp_pixel_g) & (red > blue) & (blue > green))) {
      out_pixels[offset+0] = red/2;
      out_pixels[offset+1] = (blue+red)/2;
      out_pixels[offset+2] = blue/2;
    } else {
      out_pixels[offset+0] = red;
      out_pixels[offset+1] = green;
      out_pixels[offset+2] = blue;
    }
  }
}
...
constexpr int image_pixels = width*height;
constexpr int image_cpp = 3;
constexpr int image_size = image_pixels * image_cpp;
...
public:
  MainFrame(const wxString& title, const wxPoint& pos, const wxSize& size) :
      ...
      bitmap(width, height, image_cpp*8), bitmapData(bitmap),
...
private:
  ...
  void dispatcher() {
    ...
    while(toggleLoopActive && !shuttingDown) {
      ...
      if (units <= 0) {
        ...
        soft_shrek((bus_t*)virt_buffer, (bus_t*)&virt_buffer[image_size], width, height);
      ...
      for (int unit = 0; unit < units; unit++) {
        ...
        job.accname = "Partial_shrek";
        job.params["in_pixels_1"]  = buffer + unit*width*(height/units)*3;
        job.params["in_pixels_2"]  = 0;
        job.params["out_pixels_1"] = buffer + image_size + unit*width*(height/units)*3;
        job.params["out_pixels_2"] = 0;
        job.params["image_width"]  = width;
        job.params["image_height"] = height/units;
      }
  ...
  void updateFrames() {
    copyShrekData(bitmapData, &device->buffer[image_size]);
  ...
  void copyShrekData(wxNativePixelData &dst, char *src) {
    wxNativePixelData::Iterator p(dst);
    for (int i = 0, offset = 0; i < image_pixels; i++, offset+=3) {
      p.Red() = src[offset];
      p.Green() = src[offset+1];
      p.Blue() = src[offset+2];
    }
  }
